...
When installing a Chocolatey Package, it is possible to use a number of arguments to control how the package is installed.  Each one of these arguments is detailed [here](../../chocolatey-cli/commands/install).
...
**NOTE**: Package parameters can be passed as `--params`, but also as a few different ways including `--package-parameters`. See [install options](../../chocolatey-cli/commands/install#options-and-switches):
...
**NOTE:** How you pass options with spaces varies with shell (cmd.exe vs powershell.exe) and other related information with choco options is important to keep in mind here. See [how to pass options/switches](../../chocolatey-cli/commands/#how-to-pass-options-switches).
