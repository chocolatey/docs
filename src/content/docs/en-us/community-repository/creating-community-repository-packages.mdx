...
<Callout type="info">
    If your package does not have compatibility with the <Xref title="currently supported PowerShell versions" value="chocolatey-components-dependencies-and-support-lifecycle" anchor="supported-powershell-versions" /> then you must include checks for the required PowerShell version and error if it is not installed. Alternatively, to add a dependency for the required Powershell version.
</Callout>
