...
* `centralManagementConnectionRetryMax` = **'0'** - Number of times to loop through the connection retry attempts, counted by blocks. If this value is set to zero, connection attempts will continue to be made until successful.
* `centralManagementConnectionRetrySleepInSeconds` = **'1800'** - Amount of time, in seconds, to wait before re-starting a new set of connection attempts after a block of connection attempts have all failed.
