...
<Callout type="info">    
    ...
    | Product                       | Versions Affected                                              | Notes                                                                                                            |
    |------------------------------ |--------------------------------------------------------------- | ---------------------------------------------------------------------------------------------------------------- |
    | Chocolatey Agent              | 1.1.3 and later 1.x versions, and 2.1.1 and later 2.x versions | The restricted options cannot be customized in these versions                                                    |
    | Chocolatey Licensed Extension | 5.0.4 and later 5.x versions, and 6.1.1 and later 6.x versions | The restricted options cannot be customized in these versions                                                    |
    | Chocolatey Agent              | 3.0.0 and later 3.x versions                                   | A list of restricted options can be customized using the `backgroundServiceDisallowedOptions` configuration item |
    | Chocolatey Licensed Extension | 7.0.0 and later 7.x versions                                   | A list of restricted options can be customized using the `backgroundServiceDisallowedOptions` configuration item |
</Callout>

Chocolatey CLI exposes a number of options that affect how operations are performed when the Background Service is enabled and running commands on behalf of non-administrator users.

In Chocolatey Agent 1.1.3 / 2.1.1 and Chocolatey Licensed Extension 5.0.4 / 6.1.1, these restricted options were hard-coded and could not be changed. Starting with Chocolatey Agent 3.0.0 and Chocolatey Licensed Extension 7.0.0, administrators can configure the set of restricted options used by the Background Service. The list below reflects the default restricted options that Chocolatey applies by default and will be enforced until an administrator overrides them in supported versions.

When configuring restrictions in supported versions, administrators must specify the complete set of options to restrict, including any aliases that should also be restricted.
To help minimize issues and improve security, the following options, along with their aliases, cannot be used in Chocolatey CLI or Chocolatey GUI for any operational command by default that has been enabled to run through the Background Service:

* **Default restricted options added in Chocolatey Agent 1.1.3 / 2.1.1 and Chocolatey Licensed Extension 5.0.4 / 6.1.1:**
  * `--cache-location`
  * `--install-arguments`
  * `--not-silent`
  * `--install-args-global`
  ...
  * `--install-directory`
* **Default restricted options added in Chocolatey Agent 1.1.4 / 2.1.2 and Chocolatey Licensed Extension 5.0.5 / 6.1.2:**
...
* **Default restricted options added in Chocolatey Agent 2.1.3 and Chocolatey Licensed Extension 6.2.0:**
  * `--ignore-pinned`
* **Default restricted options added in Chocolatey Agent 1.1.5 / 2.2.2 and Chocolatey Licensed Extension 5.0.8 / 6.3.1:**
  * `--trace`

<Callout type="warning">
  While most options can be added to or removed from the restricted list in Chocolatey Agent 3.0.0 and Chocolatey Licensed Extension 7.0.0, the following options are protected and cannot be overridden:
...
[![Chocolatey's Self-Service Installer - Part 1](https://img.chocolatey.org/docs/features/features_selfservice_video1.jpg)](https://www.youtube.com/watch?v=Thw9GpQaZ6M "Chocolatey's Self-Service Installer - Part 1")

* Chocolatey's Self-Service Installer - Part 2:

[![Chocolatey's Self-Service Installer - Part 2](https://img.chocolatey.org/docs/features/features_selfservice_video2.jpg)](https://www.youtube.com/watch?v=BnnAwFftUwU "Chocolatey's Self-Service Installer - Part 2")
...
![Attempting to install software as non-admin - if you are on https://docs.chocolatey.org/en-us/features/self-service-anywhere, see commented html below for detailed description of image](https://img.chocolatey.org/docs/features/features_non_admin_installer.png)
...
![Installing software with Chocolatey's background mode from the command line. - if you are on https://docs.chocolatey.org/en-us/features/self-service-anywhere, see commented html below for detailed description of image](https://img.chocolatey.org/docs/features/features_non_admin_selfservice.png)
...
![Not able to install from custom source](https://img.chocolatey.org/docs/features/features_non_admin_custom_source_error.png)
