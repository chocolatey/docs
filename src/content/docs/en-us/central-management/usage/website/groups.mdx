...
![Groups menu entry on the Chocolatey Central Management Dashboard](https://img.chocolatey.org/docs/ccm-playwright/dashboard/left-menu-groups.png)

Every Group has a Type, shown in the **Type** column on the Groups page and included in the PDF and Excel reports.
You can sort the table by this column to bring Groups of the same Type together.
The Types are described below.

## System Managed Automatic Groups

<Callout type="info">
    This feature is available in Chocolatey Central Management starting with version 0.11.0.
</Callout>

Computers are automatically added to System Managed Automatic Groups by the system.
These Groups cannot be edited or deleted.

![The All Computers Group on the Groups page with an arrow pointing to the System Automatic Type badge in the table](https://img.chocolatey.org/docs/ccm-playwright/groups/table-row-automatic-badge.png)

## System Managed Dynamic Groups

<Callout type="info">
    This feature is available in Chocolatey Central Management starting with version 0.18.0.
</Callout>

Computers are added to or removed from System Managed Dynamic Groups dynamically, according to their reported Facts.
These Groups cannot be edited, but can be <Xref title="disabled by an Administrator" value="ccm-administration-settings-groups" anchor="system-managed-dynamic-group-settings" />.

![The Operating System: Windows 11 Group on the Groups page with an arrow pointing to the System Dynamic Type badge in the table](https://img.chocolatey.org/docs/ccm-playwright/groups/table-row-dynamic-badge.png)

### Groups That May Be Created

Chocolatey Central Management builds these Groups from the Facts each Computer reports through Chocolatey Agent.
You can see the Facts a Computer has reported on the <Xref title="Facts tab of its Details page" value="ccm-computers" anchor="viewing-facts-collected-for-a-computer" />.
The operating system Facts below are all found in the **Computer System** category, within the **Operating System** group of Facts.

| Group | Fact | Computers Added to the Group |
|-------|------|------------------------------|
| `Operating System: <name>` | **Name** | Computers reporting that operating system name. One Group is created for each distinct name reported. |
| `Architecture: <architecture>` | **Architecture** | Computers reporting that architecture. One Group is created for each distinct architecture reported. |
| `Servers` | **Edition** | Computers whose reported edition contains "Server". |
| `Clients` | **Edition** | Computers whose reported edition does not contain "Server". |
| `Expiring Chocolatey Licenses` | **Expiration Date**, in the **Chocolatey License** category | Computers whose Chocolatey license expires within the next 30 days. |

A Computer is only added to a Group when it has reported the Fact that Group is built from.
A Computer that has not reported an operating system edition, for example, is added to neither the Servers Group nor the Clients Group.

A Computer whose Chocolatey license has already expired stays in the Expiring Chocolatey Licenses Group.
Chocolatey Agent stops checking in once a license has expired, so Chocolatey Central Management continues to use the expiration date that Computer last reported.

<Callout type="info">
    A System Managed Dynamic Group only appears while at least one Computer belongs to it.
    The Groups you see therefore reflect the Computers that have reported Facts, rather than a fixed list.
</Callout>

## System Managed Temporary Groups

<Callout type="info">
    This feature is available in Chocolatey Central Management starting with version 0.12.0.
</Callout>

Computers are automatically added to System Managed Temporary Groups by the system for a specific purpose, for instance as part of creating an Automatic Deployment Plan.
...
These Groups cannot be edited, deleted, or duplicated manually by a user. As well, no other Group can have such a Group as a Sub-Group.

## Creating a Group

On the main Groups page, select the **Actions for Groups** button and selecting the **Create Group** menu option.

![Create Group button on the Groups page](https://img.chocolatey.org/docs/ccm-playwright/groups/button-action-dropdown-menu-create-group.png)

Fill in a Name for the Group and (optionally) a Description in the appropriate fields in the **Create Group** page.

![Create Group Details tab](https://img.chocolatey.org/docs/ccm-playwright/groups/create/tab-details.png)
...
![Select Computer(s) or existing Group(s) tab](https://img.chocolatey.org/docs/ccm-playwright/groups/create/tab-members.png)
...
![Edit menu entry in Group actions flyout menu](https://img.chocolatey.org/docs/ccm-playwright/groups/table-row-button-action-dropdown-menu-edit.png)
...
![Details menu entry in Group actions flyout menu](https://img.chocolatey.org/docs/ccm-playwright/groups/table-row-button-action-dropdown-menu-details.png)
...
![Group Details screen showing the Computers and Groups that are currently in the Group](https://img.chocolatey.org/docs/ccm-playwright/groups/details/screen.png)
...
1. In the leftmost column of the Groups table you will find an **Actions** button which will display a **Create Deployment Plan** option.
    ...
    ![Finding the Create Deployment Plan menu entry for a specific Group on the Groups page](https://img.chocolatey.org/docs/ccm-playwright/groups/table-row-button-action-dropdown-menu-create-deployment-plan.png)
...
![Warning on Deployment Step with ineligible deployment message](https://img.chocolatey.org/docs/ccm-playwright/deployment-plans/edit/popover-ineligible-step.png)
...
![Group eligibility legend](https://img.chocolatey.org/docs/ccm-playwright/groups/create/warning-legend.png)
