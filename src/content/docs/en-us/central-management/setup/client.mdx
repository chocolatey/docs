...
import RestartServices from '@components/docs/RestartServices.mdx';
...
* There should be messages of connecting to the ccm server and checking in. It will be similar to this:
  ![Agent Setup For CCM](https://img.chocolatey.org/docs/features/ccm/agent_ccm_setup_good.png)
  Connection to report in:
  ![Agent Check-in to CCM](https://img.chocolatey.org/docs/features/ccm/agent_ccm_communication.png)
...
Central Management has specific compatibility requirements with quite a few moving parts. It is important to understand that there are some Chocolatey Agent versions that may not be able to communicate with some versions of CCM and vice versa.  Please see the <Xref title="CCM Component Compatibility Matrix" value="central-management" anchor="chocolatey-central-management-component-compatibility-matrix" /> for details.
