...
## 0.18.0 (September 16, 2026) \{#v0.18.0}

Read our [blog post](https://blog.chocolatey.org/2026/09/announcing-ccm-0180-gui-330-releases/) about this release.

### Enhancements

- Allow LDAP groups to map to CCM roles.
- Add support for OpenID Connect authentication.
- Implement System Managed Dynamic Groups.
- Prevent stored cross-site scripting vulnerabilities.
- Alphabetize social login buttons.
- Identify the source of a user account - see [Licensed #289](https://github.com/chocolatey/chocolatey-licensed-issues/issues/289).

## 0.17.0 (July 8, 2026) \{#v0.17.0}

<Callout type="warning">
    The dependencies of all the Chocolatey Central Management packages (`chocolatey-management-database`, `chocolatey-management-service`, and `chocolatey-management-web`) have changed in this release. This is to allow the installation of .NET 10.0, which is now a requirement to run Chocolatey Central Management.
</Callout>

<Callout type="warning">
    The Chocolatey Central Management Site will now _only_ work correctly when the website uses an SSL certificate.
</Callout>

Read our [blog post](https://blog.chocolatey.org/2026/07/announcing-ccm-release-0.17.0) about this release.
...
- Add support for authentication using LDAPS.
- Add support for authentication via third-party providers (Facebook, Google, Microsoft, X).
- Add support for login without a password through codes sent through email.


## 0.16.0 (May 13, 2026) \{#v0.16.0}
...
- Enable and enforce a Content Security Policy.


## 0.15.1 (April 20, 2026) \{#v0.15.1}

### Bug Fixes

- Incorrect password length restrictions on some password fields.
- ccmadmin user cannot be unlocked once it becomes locked - see [Licensed #463](https://github.com/chocolatey/chocolatey-licensed-issues/issues/463).
- Mitigate against AutoMapper security advisory.

### Enhancement

- Ensure public-facing API endpoints are documented.


## 0.15.0 (March 18, 2026) \{#v0.15.0}
...
- Include a column on Software tables for pinned status - see [Licensed #193](https://github.com/chocolatey/chocolatey-licensed-issues/issues/193).
...
- Update verbiage for consistency.
...
- Fix - When a Deployment Plan is deleted as part of retention policy, not all objects are removed from the database - see [Licensed #404](https://github.com/chocolatey/chocolatey-licensed-issues/issues/404).
...
- Chocolatey Central Management v0.3.0 will only work with Chocolatey Agent v0.11.0+. Upgrade order doesn't matter as you'll need to be on CCM v0.3.0 and Agent v0.11.0 before things start working again. See https://docs.chocolatey.org/en-us/central-management/#chocolatey-central-management-component-compatibility-matrix.
...
- Fix - Website - Ensure that minified versions of all assets are used
- Fix - Website - Ensure consistent Date/Time Formatting used everywhere
- Fix - Website - Corrected duplicate display of search input box on some screens
- Fix - Website - Error when attempting to sort by any column in table on Computer Details screen
- Fix - Website - Error when attempting to sort by Name or Package Title column in table on Software screen
- Fix - Website - Tab does not sort by outdated first on Software screen
- Fix - Website - Timezone modification doesn't provide useful information to user
- Fix - Website - Only show Software that is installed on at least one machine
- Fix - Website - Excel Export generates errors when DateTime values are included
...
- [Security] Website - Provide full RBAC to site and API
