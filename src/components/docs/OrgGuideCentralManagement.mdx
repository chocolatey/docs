...
| -------- | ----- | ----- |
| Component | `Database`, `Service`, `Web` | The component(s) of Chocolatey Central Management to install on the current server. |
...
| ConnectionString | <DynamicCodeBlockInput name="SQLConnectionString" defaultValue="Server=Localhost\SQLEXPRESS;Database=ChocolateyManagement;Trusted_Connection=true;" /> | The connection string to use to connect to the database. |
...
| CertificateThumbprint | <DynamicCodeBlockInput name="SslCertificateThumbprint" defaultValue="1234567890" /> | The thumbprint of the certificate to use for the Chocolatey Management Service. (Service and Web components only) |
...
<details>
<summary><strong>You must install the certificate to the Local Machine \ Trusted People store when providing the CertificateThumbprint. (click to expand)</strong></summary>
