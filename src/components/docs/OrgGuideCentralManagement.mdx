...
- Chocolatey Agent Service (chocolatey-agent): The service that runs on an endpoint that facilitates reporting and deployment activities from Central Management
...
Before you can install the Chocolatey Central Management Database, you need to have a SQL Server Instance available. SQL Server Express is _fine_ but only recommended for testing and small environments (less than 1000 endpoints). If you need help installing and configuring SQL Server Express, <Xref title="please see our documentation here" value="setup-sql-server-express-organizational-deployment-guide" />.
...
## Securing Central Management Communications
